...
pub fn opaque_type_preservation_test() {
  ...
  list.each(valid_gtin_8_examples(), fn(code) {
    let assert Ok(gtin_val) = gtin.from_string(code)
    gtin.to_string(gtin_val) |> should.equal(code)
    gtin.format(gtin_val) |> should.equal(gtin.Gtin8)
  })
  ...
  list.each(valid_gtin_12_examples(), fn(code) {
    let assert Ok(gtin_val) = gtin.from_string(code)
    gtin.to_string(gtin_val) |> should.equal(code)
    gtin.format(gtin_val) |> should.equal(gtin.Gtin12)
  })
  ...
  list.each(valid_gtin_13_examples(), fn(code) {
    let assert Ok(gtin_val) = gtin.from_string(code)
    gtin.to_string(gtin_val) |> should.equal(code)
    gtin.format(gtin_val) |> should.equal(gtin.Gtin13)
  })
  ...
  list.each(valid_gtin_14_examples(), fn(code) {
    let assert Ok(gtin_val) = gtin.from_string(code)
    gtin.to_string(gtin_val) |> should.equal(code)
    gtin.format(gtin_val) |> should.equal(gtin.Gtin14)
  })
}
...
pub fn property_error_messages_include_context_test() {
  ...
  list.each(invalid_lengths, fn(code) {
    let result = gtin.validate(code)
    case result {
      Error(gtin.InvalidLength(got)) -> {
...
pub fn property_specific_error_types_test() {
  ...
  list.each(invalid_check_digit_cases, fn(code) {
    let result = gtin.validate(code)
    case result {
      Error(gtin.InvalidCheckDigit) -> Nil
      _ -> {
  ...
  list.each(invalid_char_cases, fn(code) {
    let result = gtin.validate(code)
    case result {
      Error(gtin.InvalidCharacters) -> Nil
      _ -> Nil
    }
  })
  ...
  list.each(invalid_length_cases, fn(code) {
    let result = gtin.validate(code)
    case result {
      Error(gtin.InvalidLength(_)) -> Nil
      _ -> Nil
    }
  })
  ...
  let _result_prefix = gtin.gs1_prefix_country("111111111111")

  // Test InvalidFormat - normalization on non-GTIN-13
  let assert Error(gtin.InvalidFormat) = gtin.normalize("12345670")
  let assert Error(gtin.InvalidFormat) = gtin.normalize("012345678905")
}
...
pub fn integration_validation_then_prefix_lookup_test() {
  // Validate a GTIN-13
  let assert Ok(_format) = gtin.validate("6291041500213")

  // Look up the country for the same GTIN
  let assert Ok(country) = gtin.gs1_prefix_country("6291041500213")
  country |> should.equal("GS1 Emirates")

  // Test with GTIN-12
  let assert Ok(_format) = gtin.validate("012345678905")
  let assert Ok(country) = gtin.gs1_prefix_country("012345678905")
  country |> should.equal("GS1 US")

  // Test with GTIN-8
  let assert Ok(_format) = gtin.validate("96385074")
...
pub fn integration_generation_then_validation_test() {
  ...
  let assert Ok(generated_8) = gtin.generate("1234567")

  // Validate the generated GTIN-8
  let assert Ok(format) = gtin.validate(generated_8)
  format |> should.equal(gtin.Gtin8)

  // Generate a GTIN-13 from 12 digits
  let assert Ok(generated_13) = gtin.generate("629104150021")

  // Validate the generated GTIN-13
  let assert Ok(format) = gtin.validate(generated_13)
  format |> should.equal(gtin.Gtin13)

  // Generate a GTIN-14 from 13 digits
  let assert Ok(generated_14) = gtin.generate("1234567890123")

  // Validate the generated GTIN-14
  let assert Ok(format) = gtin.validate(generated_14)
  format |> should.equal(gtin.Gtin14)
}
...
pub fn integration_normalization_then_validation_test() {
  ...
  let assert Ok(normalized) = gtin.normalize("6291041500213")

  // Validate the normalized GTIN-14
  let assert Ok(format) = gtin.validate(normalized)
  format |> should.equal(gtin.Gtin14)
  ...
  let assert Ok(normalized) = gtin.normalize("5901234123457")
  let assert Ok(format) = gtin.validate(normalized)
  format |> should.equal(gtin.Gtin14)
  string.length(normalized) |> should.equal(14)
}
...
pub fn integration_full_workflow_test() {
  ...
  let assert Ok(generated) = gtin.generate(incomplete)

  // Validate the generated GTIN
  let assert Ok(format) = gtin.validate(generated)
  format |> should.equal(gtin.Gtin13)

  // Look up the country
  let assert Ok(country) = gtin.gs1_prefix_country(generated)
  country |> should.equal("GS1 Emirates")
}
...
pub fn integration_error_handling_test() {
  ...
  let result = gtin.validate("invalid")
  result |> should.be_error()

  // Try to generate with invalid length
  let result = gtin.generate("123")
  result |> should.be_error()

  // Try to normalize non-GTIN-13
  let result = gtin.normalize("12345670")
  result |> should.be_error()

  // Try to lookup prefix on invalid GTIN
  let result = gtin.gs1_prefix_country("invalid")
  result |> should.be_error()
}
...
pub fn edge_case_very_large_numbers_test() {
  let result = gtin.validate("999999999999999999999999999999")
  result |> should.be_error()

  let result = gtin.generate("999999999999999999999999999999")
  result |> should.be_error()
}
...
pub fn readme_example_validate_test() {
  // Valid GTIN-13
  let assert Ok(format) = gtin.validate("6291041500213")
  format |> should.equal(gtin.Gtin13)

  // Invalid GTIN-13
  let result = gtin.validate("6291041500214")
  result |> should.be_error()
}
...
pub fn readme_example_generate_test() {
  let assert Ok(complete) = gtin.generate("629104150021")
...
pub fn opaque_type_from_string_invalid_test() {
  // Invalid characters
  let result = gtin.from_string("629104150021A")
  result |> should.be_error()

  // Invalid length
  let result = gtin.from_string("123")
  result |> should.be_error()

  // Invalid check digit
  let result = gtin.from_string("6291041500214")
  result |> should.be_error()
}
